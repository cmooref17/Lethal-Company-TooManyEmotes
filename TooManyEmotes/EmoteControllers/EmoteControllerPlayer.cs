...
public class EmoteControllerPlayer : EmoteController
{
    ...
    {
        if (initialized && playerController != null && (playerController != HelperTools.localPlayerController || (!ConfigSettings.disableEmotesForSelf.Value)))
        {
            base.Update();
        }
    }
    ...
    protected override bool CheckIfShouldStopEmoting()
    {
        ...
        int currentSlot = playerController.currentItemSlot;
        if (currentSlot >= 0 && currentSlot < playerController.ItemSlots.Length)
        {
            GrabbableObject val = playerController.ItemSlots[currentSlot];
            if (sourceGrabbableEmoteProp != null && sourceGrabbableEmoteProp != val)
                return true;
        }
        return false;
    }
    ...
    {
        if (!initialized || ConfigSettings.disableEmotesForSelf.Value)
        {
            return false;
        }
        if (!isLocalPlayer)
        ...
        bool useSourcePropObject = false;
        int localCurrentSlot = HelperTools.localPlayerController.currentItemSlot;
        if (sourcePropObject != null && localCurrentSlot >= 0 && localCurrentSlot < HelperTools.localPlayerController.ItemSlots.Length)
        {
            useSourcePropObject = sourcePropObject == HelperTools.localPlayerController.ItemSlots[localCurrentSlot];
        }
        bool result = (!useSourcePropObject) ? PerformEmote(emote, overrideEmoteId, AudioManager.emoteOnlyMode) : PerformEmote(emote, sourcePropObject, overrideEmoteId, AudioManager.emoteOnlyMode);
    ...
    {
        if (!initialized || emoteController == null || ConfigSettings.disableEmotesForSelf.Value)
        {
            return false;
        }
        if (!isLocalPlayer)
        ...
        CustomLogging.Log("Attempting to sync emote for player: " + ((UnityEngine.Object)playerController).name + " with emote controller with id: " + emoteController.emoteControllerId);
    ...
    private static void OnSwapItem(int slot, PlayerControllerB __instance)
    {
        ...
        {
            if (slot >= 0 && slot < __instance.ItemSlots.Length)
            {
                GrabbableObject val = __instance.ItemSlots[slot];
                if (value.sourceGrabbableEmoteProp != null && value.sourceGrabbableEmoteProp != val)
                {
                    value.StopPerformingEmote();
                }
                else if (val != null && value.emotingProps.Count > 0)
                {
    ...
    public bool PerformEmote(UnlockableEmote emote, GrabbablePropObject sourcePropObject, int overrideEmoteId = -1, bool doNotTriggerAudio = false)
    {
        ...
        if (currentSlot >= 0 && currentSlot < playerController.ItemSlots.Length)
        {
            if (sourcePropObject != null && sourcePropObject == playerController.ItemSlots[currentSlot])
            {
    ...
    public override bool PerformEmote(UnlockableEmote emote, int overrideEmoteId = -1, bool doNotTriggerAudio = false)
    {
        ...
        if (isPerformingEmote)
        {
            ...
            {
                GrabbableObject val = playerController.ItemSlots[currentSlot];
                if (val != null && emotingProps.Count > 0)
                {
                    val.EnableItemMeshes(false);
                }
            }
